...
Een kracht is een **vectorgrootheid** met een grootte, richting en aangrijpingspunt, genoteerd als $\vec{F}$. Elke kracht is een wisselwerking tussen 2 voorwerpen: een **actiekracht** en een **reactiekracht**. Beide krachten zijn even groot en tegengesteld gericht, maar heffen elkaar niet op omdat ze op verschillende voorwerpen werken. Dit is de **derde wet van Newton**:
...
- **Veerkracht** ($F_\text{v}$): de kracht van een uitgerekte of samengedrukte veer, gericht in de richting van de veer.
...
- **Spankracht** ($F_\text{s}$): de kracht van een touw op een voorwerp, gericht langs het touw.
- **Zwaartekracht** ($F_\text{z}$): de kracht tussen de aarde en een massa, aangrijpend in het zwaartepunt.
...
- **Normaalkracht** ($F_\text{n}$) en **gewicht** ($F_\text{gewicht}$): het gewicht is de kracht die het object op de grond uitoefent (actiekracht). De normaalkracht is de reactiekracht van de grond, loodrecht op het oppervlak. Op een horizontaal vlak geldt $F_\text{n} = F_\text{z}$.

- **Schuifwrijvingskracht** ($F_\text{w,s}$): hangt af van de normaalkracht en de ruwheid. Er kan ook wrijving zijn zonder beweging. De wrijving past zich dan aan tot aan de **maximale schuifwrijving**:

  $$F_\text{w,s,max} = f \cdot F_\text{n}$$

  Hierin is $f$ de **wrijvingscoëfficiënt** (dimensieloos) en $F_\text{n}$ de normaalkracht (in $\mathrm{N}$).

- **Rolweerstandskracht** ($F_\text{w,r}$): hangt af van de vervorming van het rollende voorwerp (zoals een band).
- **Luchtweerstandskracht** ($F_\text{w,l}$): hangt af van de snelheid, het frontale oppervlak, de stroomlijnvorm en de luchtdichtheid.

  $$F_\text{w,l}=\frac{1}{2} \cdot \rho \cdot A \cdot c_w \cdot v^2$$

  Hierin is $\rho$ de luchtdichtheid (in $\mathrm{kg}/\mathrm{m}^3$), $A$ het frontale oppervlak (in $\mathrm{m}^2$), $c_w$ de stroomlijnfactor (dimensieloos) en $v$ de snelheid (in $\mathrm{m}/\mathrm{s}$).

### Versnelling en de wetten van Newton
...
Hierin is $a$ de versnelling (in $\mathrm{m}/\mathrm{s}^2$), $\Delta v$ de snelheidsverandering (in $\mathrm{m}/\mathrm{s}$) en $\Delta t$ het tijdsverschil (in $\mathrm{s}$). Hoe groter de nettokracht, hoe groter de versnelling. Hoe groter de massa, hoe kleiner de versnelling bij dezelfde kracht. Dit is de **tweede wet van Newton**:

$$F_\text{res} = m \cdot a$$

Hierin is $F_\text{res}$ de resulterende kracht (in $\mathrm{N}$), $m$ de massa (in $\mathrm{kg}$) en $a$ de versnelling (in $\mathrm{m}/\mathrm{s}^2$). De **traagheid** van een voorwerp geeft aan hoeveel moeite het kost om de snelheid te veranderen. Een zware massa heeft grote traagheid.

Dit zie je terug in een **v,t-diagram**: de helling is gelijk aan de versnelling. Bij een nettokracht van nul is de lijn horizontaal. Bij een constante nettokracht is de lijn recht stijgend (**eenparig versneld**) of dalend (**eenparig vertraagd**). De momentane versnelling op 1 tijdstip is de helling van de raaklijn op dat punt.

### Krachten samenstellen en splitsen

Als er meerdere krachten op een voorwerp werken, kun je die samenvoegen tot 1 resulterende kracht. Dit doe je met de **parallellogrammethode**: de 2 krachten vormen de zijdes van een parallellogram, en de diagonaal is de resulterende kracht. Als de hoek tussen de krachten 90° is, gebruik je de stelling van Pythagoras. Als 2 krachten niet op hetzelfde punt aangrijpen, verschuif je ze langs hun **werklijn** (de verlengde lijn van de kracht). Als je alleen de richting van 2 krachten en de resulterende kracht weet, kun je de **omgekeerde parallellogrammethode** toepassen.

In vectorvorm geldt:

$$\vec{F}_\text{res} = \sum_i \vec{F}_i = m \cdot \vec{a}$$

Het $\sum$-teken geeft aan dat de krachten via de parallellogrammethode worden samengesteld.
...
In een **x,t-diagram** zie je de positie op elk tijdstip. De helling op een punt is gelijk aan de snelheid op dat moment. Bij een constante snelheid is het x,t-diagram een rechte lijn; bij versnelling of vertraging een kromme.

De **gemiddelde snelheid** bereken je met:

$$v_\text{gem} = \frac{\Delta x}{\Delta t}$$

Hierin is $\Delta x$ de verplaatsing (in $\mathrm{m}$) en $\Delta t$ het tijdsverschil (in $\mathrm{s}$). Voor een eenparige beweging geldt ook $s = v \cdot t$.

De afgelegde afstand is ook af te lezen uit het v,t-diagram: de **oppervlakte onder de grafiek** is gelijk aan de afgelegde afstand.

### Vallen

Een zwaar voorwerp met een klein oppervlak valt sneller dan een licht voorwerp met een groot oppervlak. Dit komt omdat bij het zware voorwerp de zwaartekracht ($F_\text{z} = m \cdot g$) groter is, waardoor het langer duurt voordat de luchtweerstandskracht hieraan gelijk is en de nettokracht nul wordt.

Een **vrije val** is een val zonder luchtweerstandskracht. De versnelling is dan constant gelijk aan de **valversnelling** $g \approx 9{,}81\,\mathrm{m}/\mathrm{s}^2$, omdat $F_\text{z} = F_\text{res}$ geeft $m \cdot g = m \cdot a$, dus $g = a$.

De luchtweerstandskracht neemt toe met de snelheid (zie Krachten en wisselwerking). Bij een val met luchtweerstand wordt de nettokracht steeds kleiner en neemt de versnelling af. Uiteindelijk is de nettokracht nul en beweegt het voorwerp met een constante **eindsnelheid** waarbij $F_\text{z} = F_\text{w,l}$. In het v,t-diagram zie je dat de helling steeds kleiner wordt totdat de lijn horizontaal is. De afgelegde valafstand bereken je met de oppervlaktemethode.

### Omhoog en omlaag

Als je een voorwerp verticaal omhoog gooit, is de beweging eerst eenparig vertraagd (omhoog) en daarna eenparig versneld (omlaag). De snelheid omhoog is positief, omlaag negatief. De helling van het v,t-diagram is constant $-9{,}81\,\mathrm{m}/\mathrm{s}^2$. Het x,t-diagram is een parabool.

De top van de parabool in het x,t-diagram (hoogste punt) valt samen met het moment waarop het v,t-diagram de x-as snijdt (snelheid = 0).

## Gravitatie en cirkelbewegingen

### Cirkelbanen

Bij een **eenparige cirkelbeweging** heeft de baansnelheid een constante grootte maar een steeds veranderende richting. Een veranderende richting is ook een snelheidsverandering, en daarvoor is een kracht nodig. Die kracht noem je de **middelpuntzoekende kracht**: ze staat loodrecht op de baansnelheid en wijst altijd naar het middelpunt.

De middelpuntzoekende kracht is geen nieuwe kracht, maar een **rol** die een bestaande kracht vervult. In een draaimolen is dat de spankracht in het touw; bij planeten en manen is dat de gravitatiekracht.
...
De **omlooptijd** is de tijd voor 1 ronde:
...
Hierin is $F_g$ de gravitatiekracht (in $\mathrm{N}$), $G$ de **gravitatieconstante** ($6{,}674 \cdot 10^{-11}\,\mathrm{N}\,\mathrm{m}^2\,\mathrm{kg}^{-2}$), $m$ en $M$ de massa's van de 2 voorwerpen (in $\mathrm{kg}$) en $r$ de afstand tussen de zwaartepunten (in $\mathrm{m}$).

De zwaartekracht die je op aarde ervaart is de gravitatiekracht aan het aardoppervlak: $F_z = F_g$.

#### Baansnelheden van hemellichamen

Bij een cirkelbaan geldt $F_\text{mpz} = F_g$ (de gravitatiekracht levert de middelpuntzoekende kracht). Hieruit volgt de baansnelheid:
...
Bij kleine hoogteverschillen spreken we van **zwaarte-energie** ($E_z = mgh$). Bij grotere hoogteverschillen verandert de gravitatiekracht flink, en spreken we van **gravitatie-energie** ($E_g$).

De arbeid om een voorwerp te verplaatsen van $r_1$ naar $r_2$ is gelijk aan de oppervlakte onder de $F_g,r$-grafiek. De toename van de gravitatie-energie is even groot als de arbeid die verricht wordt tegen de gravitatiekracht.

Bij gravitatie-energie is het handig om het nulpunt 'in het oneindige' te leggen: een voorwerp op oneindige afstand heeft $E_g = 0$. Hierdoor heeft de gravitatie-energie altijd een negatieve waarde.
...
De **ontsnappingssnelheid** ($v_0$) is de minimale snelheid waarbij een omhooggeschoten voorwerp niet meer terugkomt. Bij de minimale ontsnappingssnelheid heeft het voorwerp in het oneindige geen kinetische energie meer en is de gravitatie-energie ook 0. Met de energievergelijking $E_{k,A} + E_{g,A} = E_{k,B} + E_{g,B}$, waarbij punt $A$ het aardoppervlak is en punt $B$ het oneindige:
...
De **kinetische energie** ($E_\text{k}$) is de energie die een voorwerp bezit door zijn beweging. Ze hangt af van de massa en de snelheid: een voorwerp met grotere massa of snelheid heeft meer kinetische energie.
...
Als je een voorwerp optilt, voeg je energie toe: er zit in een opgetild voorwerp meer energie dan in een voorwerp op de grond. Deze opgeslagen energie is **zwaarte-energie** ($E_\text{z}$). Ze kan weer vrijkomen als kinetische energie wanneer je het voorwerp laat vallen.

$$E_\text{z} = m \cdot g \cdot h$$

Hierin is $E_\text{z}$ de zwaarte-energie (in $\mathrm{J}$), $m$ de massa (in $\mathrm{kg}$), $g$ de valversnelling ($9{,}81\,\mathrm{m}/\mathrm{s}^2$) en $h$ de hoogte (in $\mathrm{m}$). Deze formule geldt ongeacht de richting van de afgelegde weg: alleen het hoogteverschil telt.
...
Om een veer te spannen is er een kracht nodig die arbeid verricht op de veer. Een gespannen veer bevat opgeslagen **veerenergie**. Ze hangt af van de veerconstante en de uitrekking:
...
Volgens de **wet van behoud van energie** kun je energie niet creëren of vernietigen, alleen omzetten. De totale hoeveelheid energie blijft altijd gelijk:

$$\sum E_\text{begin} = \sum E_\text{eind}$$

Aan de hand hiervan kun je **energievergelijkingen** opstellen: je past de wet toe op twee verschillende tijdstippen.
...
Het energieverbruik van een auto wordt gegeven in kWh per 100 km (elektrisch) of liter per 100 km (verbrandingsmotor). Het hangt af van de energiebron, het rendement van de motor en de tegenwerkende krachten.
...
Bij een grotere snelheid zijn de weerstandskrachten groter en legt het voorwerp per seconde een grotere afstand af, waardoor het benodigde vermogen snel toeneemt. Door $W = F \cdot s$ in te vullen:
...
Elektriciteit is **bewegende lading**. Verschillende ladingen trekken elkaar aan, dezelfde ladingen stoten elkaar af. De kleinste lading is de **elementaire lading** $e$ ($1{,}6 \cdot 10^{-19}\,\mathrm{C}$). Stroom kan alleen bewegen als de stroomkring gesloten is.  
...
De elektrische stroom beweegt per afspraak van plus naar min. Elektronen bewegen van min naar plus.  
...
In **combinatieschakelingen** gelden 2 wetten: de **wet van behoud van lading** (bij een splitsing blijft de totale stroom even groot) en de **wet van behoud van energie** (de energie van de bron is gelijk aan de totale energie die langs de schakeling wordt afgegeven). In een combinatieschakeling pas je de formules toe op de afzonderlijke delen.  
Als er te veel apparaten op een **groep** worden aangesloten, kan de stroomsterkte boven 16 A uitkomen: **overbelasting**. Bij **kortsluiting** raken 2 elektriciteitsdraden, waardoor de stroom een "shortcut" neemt. De weerstand wordt dan heel klein (want de stroom gaat niet meer door het apparaat) en de stroomsterkte wordt enorm groot. De **zekeringen** in een meterkast beveiligen elke groep door de stroom uit te zetten als de stroomsterkte te groot wordt.  
...
Hierin is $F_L$ de lorentzkracht (in $\mathrm{N}$), $B$ de magnetische veldsterkte (in $\mathrm{T}$), $I$ de stroomsterkte (in $\mathrm{A}$) en $l$ de lengte van de stroomdraad in het magneetveld (in $\mathrm{m}$).
...
$$U_\text{ind} \propto N \cdot \frac{\mathrm{d}\Phi}{\mathrm{d}t}$$
...
$$u(t)=A \cdot \sin\!\left(\frac{2 \pi}{T} \cdot t\right)$$
...
Hierin is $E_f$ de fotonenergie (in $\mathrm{J}$), $h$ de constante van Planck ($6{,}626 \cdot 10^{-34}\,\mathrm{J}\,\mathrm{s}$) en $f$ de frequentie (in $\mathrm{Hz}$).  
...
$$A = -\left(\frac{\Delta N}{\Delta t}\right)_{\mathrm{raaklijn}} \qquad A = -\frac{dN}{dt} \qquad A_{\mathrm{gem}} = -\frac{\Delta N}{\Delta t}$$
...
Hierin is $H$ de equivalente dosis (in $\mathrm{Sv}$), $w_R$ de **stralingsweegfactor** (dimensieloos) en $D$ de dosis (in $\mathrm{Gy}$).
