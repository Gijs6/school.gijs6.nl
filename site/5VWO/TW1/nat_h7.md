...
Een trilling kun je weergeven in een **osscillogram** (een u,t-diagram). In dit diagram zie je op elk tijdstip de **uitwijking** van **1 bepaald punt op de golf**. Je kan hieruit dus de periode van de trilling aflezen.
...
$$\vec{u}(t)=A \cdot sin(\frac{2 \pi}{T} \cdot t)$$
Hierin is $\vec{u}$ de uitwijking (in $\mathrm{m}$), $A$ de amplitude (in $\mathrm{m}$), $T$ de trillingstijd (in $\mathrm{s}$) en $t$ de tijd (in $\mathrm{s}$).

De maximale snelheid van een trillend punt is als het punt door de evenwichtsstand gaat, want dan is de helling van het u,t-diagram het stijlst. Er geldt:
...
Een **massa-veersysteem** (simpelweg een massa aan een veer) voert een **harmonische trilling** uit als je de massa naar beneden trekt en weer loslaat. Deze trilling noem je de **eigentrilling** (dus de trilling die onstaat zonder externe krachten). De frequentie van een massa-veersysteem wordt bepaald door de trillende massa en door de stugheid van de veer die de massa laat trillen.
...
En **lopende golf** beweegt zich door een tussenstof. De toppen van zo'n golf bewegen dus eigenlijk.
...
- **Longitudinale golven** zijn drukgolven: ze bewegen heen en weer door uit te rekken en samen te trekken. De trillingsrichitng is dan hetzelfde als de voortplantingsrichting, zoals geluidsgolven (zie animatie).
...
Naast u,t-diagrammen kun je ook u,x-diagrammen maken. Bij een u,t-diagram zie je van **1 punt** de uitwijking door de tijd heen. Bij een u,x-diagram zie je op alle punten (dus voor elk punt op een bepaalde afstand) de uitwijking. Een u,x-diagram is dus eigenlijk een 'foto' van de golf.  
...
Als je 2 stemvorken met net een andere frequentie beide aanslaat, hoor je steeds dezelfde toonhoogte, maar de geluidssterkte varieert. Dit afwisslend luider en zachter worden van het geluid noem je **zweven**.

Om zweven uit te leggen moeten we kijken naar **fase**. De fase ($\phi$) is het aantal trillingen dat een punt heeft gemaakt. De gereduceerde fase $\phi_\mathrm{red}$ is het aantal afgelegde trillingen vanaf het begin van de huidige trilling. Je kijkt dan dus niet naar de volledige trillingen ervoor.

Door de verschillende frequenties bij het zweven komen de golven soms **in fase** aan en soms **uit fase**. Als de golven in fase ($\Delta \phi_\mathrm{red} = 0$) aankomen, versterken ze elkaar. Als de golven uit fase ($\Delta \phi_\mathrm{red} = \frac{1}{2}$) aankomen, heffen ze elkaar juist op. Doordat de frequenties van de twee stemvorken niet gelijk zijn, verandert het *in fase* en *uit fase* steeds in elkaar.
